...
public class AppNodesBuilder {
    ...
    public TitledPane buildTitledPane(Object nodeId, 
            GridPane parentPane,int col, int row, int colSpan, int rowSpan,
            String styleClass,
            boolean enabled, Node node)  {
        TitledPane titledPane= new TitledPane();
        titledPane.getStyleClass().add(styleClass);
        ...
        titledPane.setCollapsible(enabled);
        titledPane.setExpanded(false);
        parentPane.add(titledPane, col, row, colSpan, rowSpan);
        return titledPane;
    }
